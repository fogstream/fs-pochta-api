...
echo "..... Check md formatting for PyPI"
tox -e readme
...
# remove pochta/*.egg-info/ generated during build
find . -type d -name '*.egg-info' | xargs rm -rf

source_files=`find ./pochta/ -type f | sed 's|./||'`
...
pip install --no-binary :all: -f dist/ fs-pochta-api
deactivate
rm -rf .venv/
...
pip install --only-binary :all: -f dist/ fs-pochta-api
deactivate
rm -rf .venv/
